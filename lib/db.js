...
function prepareStatements() {
  ...
  // this.getTrackingTasksStmt = this.dbConnect.prepare("SELECT * from projects WHERE tracking = 1;", printError);
  ...
  this.deleteTaskStmt = this.dbConnect.prepare("DELETE FROM projects WHERE id = ?;", printError);
  this.deleteTimeStmt = this.dbConnect.prepare("DELETE FROM times WHERE project_id = ?;", printError);
}
